...
function displayCartProduct() {
    ...
    cart.forEach((item) => {
        ...
        <tr class="cart-item">
            ...
            <td>${item.name}</td>
            <td>$ Rs {item.price.newPrice.toFixed(2)}</td>
            <td>${item.quantity}</td>
            <td>$Rs {(item.price.newPrice * item.quantity).toFixed(2)}</td>
        </tr>
        `
    }) 
...
function saveCardValues() {
    ...
    fastCargo.addEventListener("change", (e) => {
        if (e.target.checked) {
            cartTotal.innerHTML = `$ Rs{(itemsTotal + fastCargoPrice).toFixed(2)}`
        } else {
            cartTotal.innerHTML = `$ Rs{itemsTotal.toFixed(2)}`
        }
    })
}
